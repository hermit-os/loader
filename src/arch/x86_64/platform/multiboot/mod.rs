...
unsafe extern "C" fn rust_start(multiboot_info: usize) -> ! {
	MB_INFO.store(multiboot_info, Ordering::Relaxed);
	unsafe {
...
impl DeviceTree {
	pub fn create() -> FdtWriterResult<&'static [u8]> {
		...
		let multiboot = unsafe {
			Multiboot::from_ptr(MB_INFO.load(Ordering::Relaxed) as u64, &mut mem).unwrap()
		};
