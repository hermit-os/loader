...
use log::info;
use uefi::table::boot::{MemoryDescriptor, MemoryMap, MemoryType, PAGE_SIZE};
...
impl Fdt {
	...
	pub fn memory_map(mut self, memory_map: &mut MemoryMap<'_>) -> FdtWriterResult<Self> {
		memory_map.sort();
		info!("Memory map:\n{}", memory_map.display());
...
trait MemoryMapExt {
	fn display(&self) -> MemoryMapDisplay<'_>;
}
...
struct MemoryMapDisplay<'a> {
	inner: &'a MemoryMap<'a>,
}

impl<'a> fmt::Display for MemoryMapDisplay<'a> {
